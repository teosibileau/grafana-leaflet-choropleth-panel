...
Docker files (Dockerfile, entrypoint, supervisord) live in `.docker/`. The `docker-compose.yaml` in the project root references `.docker/` as the build context.

To override the default Grafana image, edit the `build.args` in `docker-compose.yaml`:
